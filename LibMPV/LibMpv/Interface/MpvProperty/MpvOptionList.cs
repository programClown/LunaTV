...
public class MpvOptionList : MpvOptionRef<IEnumerable<string>>
{
    ...
    {
        return Mpv.GetProperty<IEnumerable<string>?>(PropertyName) ?? Array.Empty<string>();
    }
    ...
    {
        return await Mpv.GetPropertyAsync<IEnumerable<string>?>(PropertyName, options) ?? Array.Empty<string>();
    }
    ...
    ///     Set a list of items (using the list separator, interprets escapes).
    /// </summary>
    public Task SetAsync(string value, MpvAsyncOptions? options = null)
    {
        return SetAsync(new[] { value }, options);
    }
    ...
    /// </summary>
    public void Clear(MpvAsyncOptions? options = null)
    {
        Mpv.ChangeList(PropertyName, ListOptionOperation.Clr, string.Empty).Invoke(options.ToCommandOptions());
    }

    /// <summary>
    ///     Clear the option (remove all items).
    /// </summary>
    public Task ClearAsync(MpvAsyncOptions? options = null)
    {
        return Mpv.ChangeList(PropertyName, ListOptionOperation.Clr, string.Empty)
            .InvokeAsync(options.ToCommandOptions());
    }
    ...
    {
        return Mpv.ChangeList(PropertyName, ListOptionOperation.Toggle, value).InvokeAsync(options.ToCommandOptions());
    }
}