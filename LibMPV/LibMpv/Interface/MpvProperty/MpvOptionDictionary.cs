...
public class MpvOptionDictionary : MpvOptionRef<IDictionary<string, string>>
{
    ...
    private string FormatKeyValueList(IDictionary<string, string> values)
    {
    ...
    {
        Mpv.ChangeList(PropertyName, ListOptionOperation.Set, FormatKeyValueList(values)).Invoke();
    }
    ...
    {
        return Mpv.ChangeList(PropertyName, ListOptionOperation.Set, FormatKeyValueList(values))
    ...
    {
        var values = Get();
        if (values != null && values.TryGetValue(key, out var result)) return result;
        return null;
    }
    ...
    /// </summary>
    public override Task AddAsync(IDictionary<string, string> values, MpvAsyncOptions? options = null)
    {
        return Mpv.ChangeList(PropertyName, ListOptionOperation.Add, FormatKeyValueList(values))
            .InvokeAsync(options.ToCommandOptions());
    }
