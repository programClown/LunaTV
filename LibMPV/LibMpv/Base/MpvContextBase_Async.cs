...
public partial class MpvContextBase
{
    ...
    private readonly object _lockLogEnabled = new();
    ...
    /// </summary>
    public MpvCommandOptions DefaultOptions { get; } = new()
    {
    ...
    public bool LogEnabled
    {
        ...
        set
        {
            lock (_lockLogEnabled)
            {
                ...
                else if (!value && _logEnabled) Log = null;
                _logEnabled = value;
            }
        }
    }
    ...
    public ulong UniqueRequestId()
    {
        return _requestId++;
    }

    /// <summary>
    ///     Gets whether to wait for response, first taking the value in options, if null taking the value in DefaultOptions,
    ///     and if null taking a default value.
    /// </summary>
    /// <param name="options">Optional command options, may be null.</param>
    public bool GetWaitForResponseOption(MpvAsyncOptions? options)
    {
        return options?.WaitForResponse ?? DefaultOptions.WaitForResponse ?? DefaultWaitForResponse;
    }

    /// <summary>
    ///     Gets the response timeout, first taking the value in options, if null taking the value in DefaultOptions, and if
    ///     null taking a default value.
    /// </summary>
    /// <param name="options">Optional command options, may be null.</param>
    public int GetResponseTimeoutOption(MpvAsyncOptions? options)
    {
        return options?.ResponseTimeout ?? DefaultOptions.ResponseTimeout ?? DefaultResponseTimeout;
    }

    /// <summary>
    ///     Gets whether to throw an exception on error, first taking the value in options, if null taking the value in
    ///     DefaultOptions, and if null taking a default value.
    /// </summary>
    /// <param name="options">Optional command options, may be null.</param>
    public bool GetThrowOnErrorOption(MpvAsyncOptions? options)
    {
        return options?.ThrowOnError ?? DefaultOptions.ThrowOnError ?? DefaultThrowOnError;
    }
    ...
    protected string[] AddCommandPrefixes(MpvCommandOptions? options, object?[] cmd)
    {
        ...
        if (cmdLength != cmd.Length || prefixCount > 0)
        {
            ...
            for (var i = 0; i < prefixCount; i++) cmd[i] = prefixes![i];
    ...
    public async Task SetPropertyAsync<T>(string name, T newValue, MpvAsyncOptions? options = null)
    {
        ...
        if (requestId > 0) await WaitForResponseAsync(requestId, name, options);
    }
    ...
    /// <param name="options">Additional command options.</param>
    private async Task<MpvEventArgs?> WaitForResponseAsync(ulong requestId, string commandName,
        MpvAsyncOptions? options = null, CancellationToken? cancelToken = null)
    {
        ...
        if (response == null)
            throw new TimeoutException(
                $"A response from MPV to request_id={requestId} was not received before timeout.");
        ...
        if (GetThrowOnErrorOption(options) && response.ErrorCode < 0) throw MpvException.FromCode(response.ErrorCode);
